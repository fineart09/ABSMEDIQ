...
export default function EditCustomer({
  ...
  // 1. เพิ่ม State สำหรับ Loading และ Error Handling (Enterprise Requirement)
  const [isLoading, setIsLoading] = useState(false);
  const [fetchError, setFetchError] = useState(null);

  // 2. ฟังก์ชันแปลงข้อมูล (Adapter)
  const mapDtoToForm = (dto) => {
    return {
      ...
      // Name Mapping
      prefixTh: dto.title || '',
      firstNameTh: dto.name || '',
      lastNameTh: dto.surname || '',
      nickname: dto.nickname || '',
      // Address Mapping (แปลงจาก Flat เป็นโครงสร้างเดิมของ UI)
      addressTh: dto.address || '',
      provinceTh: dto.province || '',
      districtTh: dto.amphur || '',
      subdistrictTh: dto.tumbon || '',
      postalCode: dto.postalCode || '', // เพิ่มใน DTO ถ้ามี
      // Details Mapping
      genderTh: dto.gender || '',
  ...
  // 3. Effect สำหรับดึงข้อมูลจาก Backend เมื่อ HN เปลี่ยนแปลง
  useEffect(() => {
    console.log("Effect Triggered! initial:", initial);

    const Hn = `${String(initial.id)}`;
    console.log("HN:", Hn);
    ...
    const fetchCustomerData = async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Error: ${response.status} - ไม่พบข้อมูลลูกค้า`);
        }

        const data = await response.json();

        // Update Form State ด้วยข้อมูลที่ Map แล้ว
        const mappedData = mapDtoToForm(data);
        setForm(mappedData);
    ...
    fetchCustomerData();
  }, [initial?.hn]); // รันใหม่เมื่อ HN เปลี่ยน
